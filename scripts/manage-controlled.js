...
class ControlledManager {
  ...
  async getCurrentList() {
    console.log('📋 Pobieranie aktualnej listy kontroli SANEPID...\n');
    
    try {
      const response = await fetch(`${this.workerUrl}/controlled`);
      const data = await response.json();
      
      if (data.success) {
        console.log(`✅ Aktualne kody pod kontrolą SANEPID (${data.codes.length}):`);
  ...
  async updateList(codes, token) {
    console.log(`🔄 Aktualizowanie listy kontroli SANEPID (${codes.length} kodów)...\n`);
    
    try {
      const response = await fetch(`${this.workerUrl}/controlled/update`, {
        method: 'POST',
        headers: {
      ...
      if (data.success) {
        console.log('✅ Lista kontroli SANEPID zaktualizowana pomyślnie!');
  ...
  printHelp() {
    console.log(`
🎯 Menedżer kodów pod kontrolą SANEPID - HS Code Verifier

Użycie:
  node scripts/manage-controlled.js [polecenie] [opcje]

Polecenia:
  list                    - Wyświetl aktualną listę kodów
  update <token> <pliki>  - Zaktualizuj listę kontroli SANEPID
  help                    - Wyświetl tę pomoc

Przykłady:
  node scripts/manage-controlled.js list
  node scripts/manage-controlled.js update "TWÓJ_TOKEN" controlled-list.txt
  node scripts/manage-controlled.js update "TWÓJ_TOKEN" list1.txt list2.txt
    ...
    switch (command) {
      ...
      case 'update':
        if (args.length < 3) {
          ...
          console.log('   Użyj: node scripts/manage-controlled.js update <token> <pliki...>');
          process.exit(1);
        }
        ...
        if (confirm.toLowerCase() === 'tak') {
          await this.updateList(allCodes, token);
        } else {
